...
class ServiceController {
    ...
    async get_products({request, response}) {
        let { p_page, p_search, p_currentSort, p_currentSortDir } = request.post(['p_page', 'p_search', 'p_currentSort', 'p_currentSortDir'])
        try {
            let [res, page_count] = await ServicesMod.get_products(p_page, p_search, p_currentSort, p_currentSortDir)
    ...
    async get_products_unpublished({request, response}) {
        let { p_page, p_search, p_currentSort, p_currentSortDir } = request.post(['p_page', 'p_search', 'p_currentSort', 'p_currentSortDir'])
        try {
            let [res, page_count] = await ServicesMod.get_products_unpublished(p_page, p_search, p_currentSort, p_currentSortDir)
            return response.status(200).send({res, page_count})
        } catch(e) {
            console.log(e)
            return response.status(408).send(e.message)
    ...
    async update_products_unpublished({request, response}) {
        ...
        try {
            ...
            if(pub_id.length == list_of_ids.length) {
                ...
                return response.status(408).send({ msg: 'an error occured while updating the status of products.' })
            }
    ...
    async update_products_status({request, response}) {
        let { product_id, location_id, location_name, user_id } = request.post(['product_id', 'location_id', 'location_name', 'user_id'])
        try {
            let data = {
                status: 'draft'
            }

            let prodUpdate = await this.WooCommerce.put(`products/${product_id}`, data)
            if(prodUpdate.status == 200) {
    ...
    async get_orders({request, response}) {
        let { p_page, p_search, p_currentSort, p_currentSortDir, p_orderType } = request.post(['p_page', 'p_search', 'p_currentSort', 'p_currentSortDir', 'p_orderType'])
        let location_name = 'srs-novaliches'
        try {
            let [res, page_count] = await ServicesMod.get_orders(p_page, p_search, p_currentSort, p_currentSortDir, p_orderType)
            // let orders = []
    ...
    async update_product_excluded({request, response}) {
        let { product_id, product_price, product_price_ws, product_mode, product_mode_ws, location_id, location_name, user_id } = request.post(['product_id', 'product_price', 'product_price_ws', 'product_mode', 'product_mode_ws', 'location_id', 'location_name', 'user_id'])
        try {
            if(product_mode == 1) {
                let data = []
                ...
                if(prodUpdate.status == 200) {
                    let row = await ServicesMod.update_product_excluded(product_id, product_mode)
                    if(row) {
                        await ServicesMod.audit_trail(user_id, location_id, location_name, `MANUAL PRODUCT PRICING ${product_price}`)
                ...
                let row = await ServicesMod.update_product_excluded(product_id, product_mode)
                if(row) {
                    await ServicesMod.audit_trail(user_id, location_id, location_name, `AUTO PRODUCT PRICING ${product_price}`)
                    return response.status(200).send(true)
                }
            }
    ...
    async get_users({request, response}) {
        let { p_page, p_search, p_currentSort, p_currentSortDir } = request.post(['p_page', 'p_search', 'p_currentSort', 'p_currentSortDir'])
        try {
            let [ resF, page_count ] = await ServicesMod.get_users(p_page, p_search, p_currentSort, p_currentSortDir)
            let res = []
    ...
    async add_user({request, response}) {
        let { username, userpass, userrole, userbranch, userfirstname, userlastname } = request.only(['username', 'userpass', 'userrole', 'userbranch', 'userfirstname', 'userlastname'])
        try {
            let res = await ServicesMod.add_user(username, userpass.replace(/\s/g,''), userrole, userbranch, userfirstname, userlastname)
            if(res) {
    ...
    async update_user({request, response}) {
        let { user_id, password, user_role } = request.only(['user_id', 'password', 'user_role'])
        try {
    ...
    async get_total_order_amount_month({request, response}) {
        let { branch, account_no } = request.only(['branch', 'account_no'])
        try {
            let row = await ServicesMod.get_total_order_amount_month(branch, account_no)
            return response.status(200).send({row})
        } catch (e) {
            console.log(e)
            return response.status(408).send(e.message)
    ...
    async get_total_order_count_month({request, response}) {
        let { branch, account_no } = request.only(['branch', 'account_no'])
        try {
            let row = await ServicesMod.get_total_order_count_month(branch, account_no)
            return response.status(200).send({row})
        } catch (e) {
            console.log(e)
            return response.status(408).send(e.message)
    ...
    async get_list_of_transactions({request, response}) {
        let { p_page, p_currentSort, p_currentSortDir } = request.only(['p_page', 'p_currentSort', 'p_currentSortDir'])
        let account_no = '4272444737'
        let branch = 'SRSMNOVA'
        try {
            let [res, count] = await ServicesMod.get_list_of_transactions(branch, account_no, p_page, p_currentSort, p_currentSortDir)
    ...
    async get_transaction_details({request, response}) {
        let { TransactionNo, TerminalNo, branch } = request.only(['TransactionNo', 'TerminalNo', 'branch'])
        try {
            let res = await ServicesMod.get_transaction_details(TransactionNo, TerminalNo, branch)
            return response.status(200).send({res})
        } catch(e) {
            console.log(e)
            return response.status(408).send(e.message)
    ...
    async custom_batch_upload({request, response}) {
        ...
        await upload.move(Helpers.publicPath(dir), { name: fileName, overwrite: true })
        ...
        for(let i = 0; i<= explanation.rowCount; i++) {
            if(i >= 2) {
                if(explanation.getCell('A'+i).value == '' || explanation.getCell('A'+i).value == null) continue
                if(explanation.getCell('C'+i).value == 'R') {
                    if(explanation.getCell('O'+i).value == '' || explanation.getCell('O'+i).value == null) continue
                    ...
                    await this.createRProduct(explanation.getCell('A'+i).value, explanation.getCell('B'+i).value.toUpperCase(), explanation.getCell('E'+i).value.toUpperCase(), explanation.getCell('F'+i).value, explanation.getCell('G'+i).value, explanation.getCell('H'+i).value, explanation.getCell('I'+i).value, explanation.getCell('J'+i).value, explanation.getCell('K'+i).value, explanation.getCell('L'+i).value, explanation.getCell('M'+i).value, explanation.getCell('N'+i).value, explanation.getCell('O'+i).value, explanation.getCell('Q'+i).value, explanation.getCell('R'+i).value, explanation.getCell('S'+i).value)
    ...
    async createRProduct(sku, productName, shortDescription, mainCat1, subCat1, mainCat2, subCat2, mainCat3, subCat3, mainCat4, subCat4, uom, barcode_ret, grams, by_barcode, imageUrl) {
        try {
            let prod_details_ret
            if(by_barcode == 'Y' || by_barcode == 'Yes' || by_barcode == 'yes' || by_barcode == 'y') {
                let temp_sku = sku
                let temp_barcode = barcode_ret
                sku = temp_barcode
                barcode_ret = temp_sku
                prod_details_ret = await ServicesMod.getStockPriceBarcode(sku.toString(), barcode_ret.toString())
            } else {
                prod_details_ret = await ServicesMod.getStockPrice(sku.toString(), barcode_ret.toString())
            }
            ...
            const data = {
                ...
                type: 'simple',
                stock_quantity: (parseInt(prod_details_ret.SellingArea) > 0) ? parseInt(prod_details_ret.SellingArea) : 0,
        ...
        try {
            ...
            for(let i = 1; i<=67; i++) {
                ...
                for(const rows of res) {
                    ...
                    await ServicesMod.insertProductDB(rows.id, rows.sku, rows.name, barcode, i, grams, by_barcode, uom)
                }
            }
            // UPDATE PAGE COUNTER
